...
export default function SearchBar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <form onSubmit={submit} role="search" className="w-full">
      <label className="sr-only" htmlFor={compact ? "header-search" : "catalog-search"}>Search products</label>
      <div className="flex min-h-11 items-center gap-2 rounded-xl border border-(--color-border) bg-(--color-surface) px-3 transition focus-within:border-(--color-border-strong) focus-within:ring-2 focus-within:ring-(--color-primary-subtle)">
        ...
        />
        {value && (
          <button type="button" onClick={clear} aria-label="Clear search" className="inline-flex min-h-8 min-w-8 items-center justify-center rounded-md text-(--text-muted) hover:bg-(--color-primary-subtle) hover:text-(--text-high-emphasis)">
